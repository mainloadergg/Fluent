...
function Video:New(_, config)
	...
	local wrap = New("Frame", { Size = UDim2.new(1, -16, 0, hasVideo and 220 or 150), BackgroundTransparency = 0.85, ClipsDescendants = true, Parent = parent, ThemeTag = { BackgroundColor3 = "Element" } })
	...
	if hasVideo then
		...
		vid.Size = UDim2.new(1, 0, 0, 152); vid.BackgroundTransparency = 1; vid.Looped = looped; vid.Volume = vol; vid.Video = resolved; vid.Parent = wrap
		Instance.new("UICorner", vid).CornerRadius = UDim.new(0, radius)
		local controls = New("Frame", { Size = UDim2.new(1, 0, 0, 30), Position = UDim2.new(0, 0, 0, 156), BackgroundColor3 = Color3.fromRGB(0, 0, 0), BackgroundTransparency = 0.35, ZIndex = 4, Parent = wrap })
		...
		local function ctrlBtn(label, cb)
			local btn = New("TextButton", { Size = UDim2.fromOffset(60, 22), BackgroundTransparency = 0.7, Text = label, TextSize = 11, ZIndex = 5, Parent = controls, ThemeTag = { BackgroundColor3 = "Tab", TextColor3 = "Text" } })
			New("UICorner", { CornerRadius = UDim.new(0, 4), Parent = btn })
			Creator.AddSignal(btn.MouseButton1Click, function() pcall(cb) end)
			return btn
		end
		ctrlBtn(" Play", function() vid:Play() end)
		ctrlBtn(" Pause", function() vid:Pause() end)
		ctrlBtn(" Stop", function() vid:Stop() end)
		if auto and resolved ~= "" then task.spawn(function() task.wait(0.05); if vid and vid.Parent then vid:Play() end end) end
	else
		New("TextLabel", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "No video — use rbxassetid://", TextSize = 12, TextTransparency = 0.4, Parent = wrap, ThemeTag = { TextColor3 = "SubText" } })
	end
	local mod = { Frame = wrap, Type = "Video", VideoFrame = vid }
	function mod:SetVideo(v) local r = resolveSync(v); if vid then vid.Video = r end end
