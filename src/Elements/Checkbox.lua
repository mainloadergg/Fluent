...
function Checkbox:New(idx, config)
	...
	local t = { Value = config.Default and true or false, Callback = config.Callback or function() end, Type = "Checkbox" }
	local el = require(Root.Components.Element)(config.Title, config.Description, self.Container, true)
	t.SetTitle = el.SetTitle; t.SetDesc = el.SetDesc; t.Frame = el.Frame
	local box = New("Frame", {
		Size = UDim2.fromOffset(20, 20), AnchorPoint = Vector2.new(1, 0.5),
		Position = UDim2.new(1, -10, 0.5, 0), Parent = el.Frame,
	...
	local check = New("ImageLabel", {
		...
		Image = "rbxassetid://10709790644", ImageTransparency = 1,
		ThemeTag = { ImageColor3 = "CheckboxCheck" }, Parent = box,
	})
	function t:OnChanged(cb) t.Changed = cb; cb(t.Value) end
	function t:SetValue(val)
		val = not not val; t.Value = val
		...
		lib:SafeCallback(t.Callback, val); lib:SafeCallback(t.Changed, val)
	end
	function t:Destroy() el.Frame:Destroy(); lib.Options[idx] = nil end
	Creator.AddSignal(el.Frame.MouseButton1Click, function() t:SetValue(not t.Value) end)
	t:SetValue(t.Value); lib.Options[idx] = t; return t
end
return Checkbox
