...
function Input:New(idx, config)
	...
	local h = { Value = config.Default or "", Numeric = config.Numeric or false, Finished = config.Finished or false, Callback = config.Callback or function() end, Type = "Input" }
	local el = require(Root.Components.Element)(config.Title, config.Description, self.Container, false)
	h.SetTitle = el.SetTitle; h.SetDesc = el.SetDesc; h.Frame = el.Frame
	...
	function h:SetValue(val)
		...
		h.Value = val; box.Text = val
		lib:SafeCallback(h.Callback, val); lib:SafeCallback(h.Changed, val)
	end
	function h:OnChanged(cb) h.Changed = cb; cb(h.Value) end
	function h:Destroy() el.Frame:Destroy(); lib.Options[idx] = nil end
	if h.Finished then
		Creator.AddSignal(box.FocusLost, function(enter) if enter then h:SetValue(box.Text) end end)
	else
		Creator.AddSignal(box:GetPropertyChangedSignal("Text"), function() h:SetValue(box.Text) end)
	end
	lib.Options[idx] = h; return h
end
return Input
